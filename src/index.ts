...
// app.use(bodyparser.urlencoded({ extended: true }));  // use when your data pass from HTML from
...
io.on("connection", (socketIo) => {
  ...
  socketIo.on("htmlMyEve", (data) => {  //msg userid
    switch (data.message) {
      case method.question1:
        userId = data.userId
        io.emit("serMsgEvent", {message:method.question1,userId});
        chatBoatController.chatCreate(chatId, userId, adminId, method.question1).then(()=>{
        ...
        case method.question2:
          ...
          chatBoatController.chatCreate(chatId, userId, adminId, method.question2).then(()=>{
...
//here change httpServer.listen to app.listen
