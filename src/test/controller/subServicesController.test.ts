...
describe("SubServicesController", () => {
  ...
  it("15 should return success if work data is soft deleted", async () => {
    (AppDataSource.getRepository(SubServices).softDelete as jest.Mock).mockResolvedValueOnce({ affected: 1 });

    mockRequest.params = { id: "1" };

    await subServicesController.removeSubServices(mockRequest as Request, mockResponse as Response);
  ...
  it("16 should return not found if work data does not exist", async () => {
    (AppDataSource.getRepository(SubServices).softDelete as jest.Mock).mockResolvedValueOnce({ affected: 0 });
  ...
  it("17 should return server error on unexpected error", async () => {
    (AppDataSource.getRepository(SubServices).softDelete as jest.Mock).mockRejectedValueOnce(new Error("Unexpected error"));
